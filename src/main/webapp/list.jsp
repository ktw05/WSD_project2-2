...
<%-- 1. DAO를 사용하여 DB에서 데이터 목록 가져오기 --%>
<%
    BoardDAO dao = BoardDAO.getInstance();
    List<BoardVO> posts = dao.getAllPosts();
...
<%-- 2. 검색 기능 영역 개선: d-flex-wrap 추가 및 아래쪽 마진(mb-4) 추가 --%>
<form action="list.jsp" method="get" class="d-flex flex-wrap mb-4">
    <input class="form-control me-2 flex-grow-1 mb-2 mb-sm-0" type="search" name="searchKeyword" placeholder="제목, 작성자 검색" aria-label="Search" style="max-width: 300px;">
    <button class="btn btn-outline-success" type="submit">검색</button>
</form>

<%-- 3. 목록 출력 (테이블 반응성 wrapper 추가) --%>
<div class="table-responsive">
    <table class="table table-hover table-striped">
        ...
        <c:if test="${empty postList}">
            <tr>
                <td colspan="6" class="text-center p-5 text-muted">등록된 게시글이 없습니다.</td>
            </tr>
        </c:if>
